...
class GoogleDocs4MW {
	...
	public static function renderGoogleSpreadsheet( $input, $argv ) {
		...
		$height = isset( $argv['height'] ) ? $argv['height'] : 800;
        $style = isset( $argv['style'] ) ? $argv['style'] : 'width:100%';
        $headers = isset( $argv['headers'] ) ? $argv['headers'] : 'false';
		...
		$output = '<iframe class="googlespreadsheetframe" width="' .
				...
				'" src="http://docs.google.com/spreadsheets/d/e/' . $key .
                '/pubhtml?widget=true&amp;headers=' . $headers . 
                '"></iframe>';
